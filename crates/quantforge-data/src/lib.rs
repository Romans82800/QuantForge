...
impl QuoteBarDataset {
    ...
    /// Drop quote bars at or after `end_ms` (UTC), preserving the hash.
...
impl BarDataset {
    ...
    /// Drop bars at or after `end_ms` (UTC), preserving the hash.
    pub fn trim_at_or_after_timestamp_ms(&mut self, end_ms: i64) -> Result<usize, DataError> {
        let keep_to = self.bars.partition_point(|bar| bar.timestamp_ms < end_ms);
        if keep_to == 0 {
            return Err(DataError::EmptyAfterHistoryTrim { start_ms: end_ms });
...
/// UTC millisecond instant of broker-local `date` at midnight.
pub fn history_date_cutoff_ms(source_timezone: &str, date: &str) -> Result<i64, DataError> {
    let timezone: SourceTimezone = source_timezone.parse()?;
    let parsed = NaiveDate::parse_from_str(date, "%Y-%m-%d").map_err(|_| DataError::InvalidTimestamp {
        row: 0,
        value: date.to_owned(),
...
pub fn history_end_exclusive_cutoff_ms(source_timezone: &str, date: &str) -> Result<i64, DataError> {
    let parsed = NaiveDate::parse_from_str(date, "%Y-%m-%d").map_err(|_| DataError::InvalidTimestamp {
        row: 0,
        value: date.to_owned(),
        reason: "expected an ISO date in YYYY-MM-DD format",
    })?;
    let next = parsed.checked_add_signed(Duration::days(1)).ok_or_else(|| DataError::InvalidTimestamp {
        row: 0,
        value: date.to_owned(),
